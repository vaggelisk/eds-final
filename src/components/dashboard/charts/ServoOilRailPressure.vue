<template>
    <v-responsive>
        ...
        <v-card-title primary class="title"> Servo Oil Rail Pressure</v-card-title>
            ...
            <v-layout row wrap>
             <v-flex v-show="isShowing" xs4>
              ...
              <v-flex v-if="isShowing" xs8>
                  <canvas id="dot-chart-servo-1" @click="isShowing ^= true"></canvas>
              </v-flex>
              <v-flex v-else xs12>
                   <canvas id="dot-chart-servo-1" @click="isShowing ^= true"  ></canvas>
              </v-flex>
...
<script>
    ...
    export default {
        ...
        components: {DxButton,},
        props: ['childServoOilRailPressureDataLoaded', 'ServoOilRailPressureData'],
        data: function () {
            return {
                isShowing: true,
                text: "Enlarge",
        ...
        mounted() {
          this.loading = false;
          this.createChart2('dot-chart-servo-1', this.dotsChartData);
